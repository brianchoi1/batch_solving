...
class Ui_MainWindow(QWidget):
    ...
    def setupUi(self, MainWindow):
        ...
        # self.tableWidget.setItem(0, 2, QTableWidgetItem('input_lsdyna.k'))
        # self.tableWidget.setItem(0, 3, QTableWidgetItem('8'))
        # self.tableWidget.setItem(0, 4, QTableWidgetItem('lsdyna ncpu=8 i=input_lsdyna.k'))

        # progressbar = self.progress_bar()
        # chkbox = self.chkbox()  
        # scratch_ckkbox = self.scratch_chkbox()
        # pshbtn = self.pshbtn()
        # include_btn = self.include_btn()
    ...
    def inp_open_Clicked(self):
        try:
            ...
            for index, (path1, path2) in enumerate(zip(self.fname_list, fname)):
                id = index + 1
                globals()["table_list_{}".format(id)] = {"id" : id, "filename" : path1, "filepath" : path2, "ncpu" : 8, "postfix" : '', "run_cmd" : '', "include_file" : '', "include_file_path" : '', "solve_key" : 0, "running_key" : 0, "error_key" : 0, "bkill_key" : 0, "status" : '', "status_progress" : '', "scratch" : 1}
                self.list_table_list.append("table_list_{}".format(id))
    ...
    def file_distributor(self):
        ...
        for table_list in self.list_table_list:
    ...
    def scratch_chkbox(self):
        ckbox = QCheckBox(self.tableWidget)
        ckbox.setCheckState(Qt.Checked)
        self.tableWidget.setCellWidget(0, 7, ckbox)
    
    def pshbtn(self):
        btn1 = QPushButton("X", self.tableWidget)
        self.tableWidget.setCellWidget(0, 8, btn1)

    def include_btn(self):
        btn1 = QPushButton("include", self.tableWidget)
        self.tableWidget.setCellWidget(0, 5, btn1)
