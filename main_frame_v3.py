...
class Ui_MainWindow(QWidget):
    ...
    def setupUi(self, MainWindow):
        ...
        self.pushButton_7.clicked.connect(self.upload_func_ready)
        self.progressChanged.connect(self.progress_bar)
    ...
    def inp_open_Clicked(self):
        try:
            ...
            for index, (path1, path2) in enumerate(zip(self.fname_list, fname)):
                id = index + 1
                globals()["table_list_{}".format(id)] = {"id" : id, "filename" : path1, "filepath" : path2, "ncpu" : 8, "postfix" : '', "run_cmd" : 'lsdyna ncpu=8 i=' + path1, "include_file" : '', "include_file_path" : '', "solve_key" : 0, "running_key" : 0, "error_key" : 0, "bkill_key" : 0, "status" : '', "status_progress" : 0, "status_progress_solving" : 0, "scratch" : 1}
    ...
    def progress_bar(self, table_list):
        ...
        value = (table_list['status_progress']) * 100
        pbar.setValue(int(value))    
        pbar.setFormat('%.02f%%' % (value))
        pbar.setAlignment(Qt.AlignCenter)
        self.tableWidget.setCellWidget(table_list['id']-1, 6, pbar)

    def progress_bar_solving(self, table_list):
        pbar = QProgressBar(self.tableWidget)
        COMPLETED_STYLE = """QProgressBar::chunk { background-color: red; }"""
        pbar.setStyleSheet(COMPLETED_STYLE)
        value = (table_list['status_progress']) * 100
        pbar.setValue(int(value))    
        pbar.setFormat('%.02f%%' % (value))
        pbar.setAlignment(Qt.AlignCenter)
    ...
    def upload_func_ready(self):
        tt = threading.Thread(target=self.upload_func)
        tt.start()
        ...
        if self.login_key == 1:
            ...
            for table_list in self.group_table_list:
                ...
                if table_list['scratch'] == 1:
                    ...
                    th.start()
                    time.sleep(5)
                    thr = threading.Thread(target=self.upload_check, args=(('cd scratch/' + self.default_fld + '/' + input_fld), str(table_list['filepath']), str(table_list['filename']), str(table_list['ncpu']), table_list))
                    thr.start()
                else:
                    t = open('upload_cmd', 'w', encoding='utf-8')
                    ...
                    th.start()
                    time.sleep(5)
                    thr = threading.Thread(target=self.upload_check, args=(('cd ' + self.default_fld + '/' + input_fld), str(table_list['filepath']), str(table_list['filename']), str(table_list['ncpu']), table_list))
                    thr.start()
                # self.run()
                # self.ssh.close()
    ...
    def upload_check(self, hpc_path, filepath, fname, ncpu, table_list):
        self.ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy()) 
        self.ssh.connect(self.ip, 22, self.id, self.pw)
        i = 1
        while i > 0:
            ...
            try:
                ...
                table_list['status_progress'] = ratio
                # progressbar = self.progress_bar(table_list)
                self.progressChanged.emit(table_list)
                print("%.2f%%" % (ratio * 100))
                if ratio == 1:
                    i -= 1
                    stdin, stdout, stderr = self.ssh.exec_command(hpc_path + '; . /etc/profile;. ~/.bash_profile;. ~/.bashrc; lsdyna i=' + fname + ' ncpu=' + ncpu + ' ' + self.postfix_data)
                time.sleep(10)
            except:
                time.sleep(10)
                continue
